...
if __name__ == '__main__':
    ...
    for i_sample in range(1000):
        ...
        theta_expert = stats.truncnorm.rvs(a, b,
                                           ...
                                           size=n_feature)
        # Generate trajectories
        r_s_expert = np.squeeze(np.asarray(np.dot(state_feature, 
            theta_expert[:n_state_feature])))
        r_a_expert = np.squeeze(np.asarray(np.dot(action_feature, 
            theta_expert[n_state_feature:])))
        V, Q, policy_expert = vi_boltzmann(mdp, gamma, r_s_expert, 
                                           r_a_expert, traj_len, temperature)
        behavior_instances = generate_trajectories(mdp, policy_expert, 
                                                   traj_len, n_traj)
        behavior_state = behavior_instances[:,:,0].flatten()
        behavior_action = behavior_instances[:,:,1].flatten()
        
        # Create folders to store results
        if not os.path.isdir(path_store + '/info'):
            os.mkdir(path_store + '/info')
        if not os.path.isdir(path_store + '/trace'):
            os.mkdir(path_store + '/trace')
        if not os.path.isdir(path_store + '/KL'):
            os.mkdir(path_store + '/KL')

        # Store information of this run
        info = {}
        info['theta'] = theta_expert
        info['ISD'] = initial_state_dist[valid_init]
        info['behavior_instances'] = behavior_instances
        np.save(path_store + '/info/info_{}_{}.npy'.format(filename,i_sample),
                [info])
